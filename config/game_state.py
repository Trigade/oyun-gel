...
class GameState(State):
    def __init__(self, manager):
        ...
        # Görsel ve ses kaynaklarının yüklenmesi
        ...
        self.ready_image = pygame.transform.scale(
            (pygame.image.load(r"images\text_ready.png")), (200, 60)
        )
        self.tutorial_image = pygame.transform.scale(
            (pygame.image.load(r"images\tutorial.png")), (120, 100)
        )
        self.pause_btn = Button(r"images\button_pause.png", 360, 40, 40, 40)

        # Skor rakamları listesi
        self.number_images = [
            pygame.transform.scale(
                pygame.image.load(f"images/{i}.png").convert_alpha(), (30, 55)
            )
            for i in range(10)
        ]
        ...
        # Nesne gruplarının oluşturulması
    ...
    def spawn_pipes(self):
        """Alt ve üst boruları rastgele boşlukla oluşturur."""
        x_pos = 450
        gap_size = 150
    ...
    def handle_events(self, events):
        """Klavye ve fare girişlerini kontrol eder."""
        for event in events:
            ...
            if self.pause_btn.is_clicked(event) or (
                ...
                if not self.is_paused:
                    ...
                    # Duraklatılan süreyi boru zamanlayıcısına ekleyerek senkronu korur
                    pause_duration = pygame.time.get_ticks() - self.pause_start_time
                    self.pipe_timer += pause_duration
                    self.is_paused = False
                return

            # Zıplama ve oyun başlatma
            if not self.is_paused:
    ...
    def update(self):
        """Oyun mantığını (hareket, çarpışma, skor) günceller."""
        ...
        if self.game_started and not self.is_game_over:
            ...
            # Çarpışma kontrolleri (Boru, Yer, Tavan/Taban)
            ...
            # Oyun bittiğinde ekran görüntüsü al ve duruma geç
            if self.is_game_over:
                self.manager.music.stop_music()
                self.manager.music.play_sound("hit")
    ...
    def draw(self, screen):
        ...
        if self.is_paused:
            ...
            screen.blit(p_surf, p_surf.get_rect(center=(200, 300)))

            hint_surf = self.font.render(
                "Devam etmek için butona basın", True, (200, 200, 200)
            )
    ...
    def draw_score(self, screen):
        """Skoru rakam görsellerini kullanarak ekrana ortalar."""
        score_str = str(self.score)
        total_width = sum(
            self.number_images[int(char)].get_width() + 2 for char in score_str
        )
