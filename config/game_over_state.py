...
class GameOverState(State):
    def __init__(self, manager, final_score, screenshot):
        ...
        # Arayüz görselleri ve butonlar
        self.over_image = pygame.transform.scale(
            pygame.image.load(r"images\text_game_over.png"), (200, 60)
        )
        self.panel_image = pygame.transform.scale(
            pygame.image.load(r"images\score_panel.png"), (240, 126)
        )
    ...
    def handle_events(self, events):
        """Tıklama ve çıkış işlemlerini yönetir."""
        for event in events:
    ...
    def draw(self, screen):
        ...
        # Mevcut skor ve rekoru panel üzerine çizer
    ...
    def draw_panel_score(self, screen, score, y_pos):
        """Skoru rakam görsellerini kullanarak hizalı şekilde çizer."""
        score_str = str(score)
        padding = 2
        total_width = (len(score_str) * self.small_digit_width) + (
            (len(score_str) - 1) * padding
        )
